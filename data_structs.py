...
class Vocabulary(object):
    ...
    def decode(self, matrix):
        ...
        for i in matrix:
            if i == self.vocab['EOS']:
                break
            tok = self.reversed_vocab[i]
            if tok == 'GO':           # omit 'GO' token in decoded SMILES
                continue
            chars.append(tok)
        smiles = "".join(chars)
...
class Inception(object):
    ...
    def add_experience(self, smiles, score, likelihood, scaffold, scaf_fp):
        ...
        self.memory = pd.concat([self.memory, df], axis=0)
        self._purge_memory()
    ...
    def update_score(self, smiles, score):
        scaffold = [self._calculate_scaffold(smi) for smi in smiles]
        scaf_fp = [None if scaf is None or Chem.MolFromSmiles(scaf) is None else Morgan(Chem.MolFromSmiles(scaf), 2, 1024) for scaf in scaffold]
    ...
    def _find_similar_scaffold(self, scaffold):
        ...
        """
        if scaffold is not None and Chem.MolFromSmiles(scaffold) is not None:
...
def construct_vocabulary(smiles_list, voc_surfix=None):
    ...
    if voc_surfix is not None:
        with open(f'data/Voc-{voc_surfix}', 'w') as f:
...
if __name__ == "__main__":
    ...
    #-----------------------------------------------------
    # Construct vocabulary for kinase datasets
    #-----------------------------------------------------
    KINASES = [
    ...
    for kinase in KINASES:
        smiles_file = f"data/kinase/{kinase}/keepmean-norepeat_{kinase}.smi"
        print(f"Reading {kinase} smiles...")
        smiles_list = canonicalize_smiles_from_file(smiles_file)
        print("Constructing vocabulary...")
        voc_chars = construct_vocabulary(smiles_list, voc_surfix=kinase)
        write_smiles_to_file(smiles_list, f"data/kinase/{kinase}/keepmean-norepeat_{kinase}_filtered.smi")